...
实现一个构造3位二进制数乘方电路，输入为3位二进制数X2、X1、X0，输出为6位二进制数Y5、Y4、Y3、Y2、Y1以及Y0。实现的函数为y=x^2，请构造一个真值表，写出Y5～Y0的逻辑函数，并根据逻辑函数实现一个简易乘方电路，并自行仿真验证其正确性。

## 简易比较电路

实现一个比较两个2位二进制数的电路，输入为两个2位二进制数：A1、A0以及B1、B0，输出为F2、F1、F0，F2为真代表A>B，F1为真代表A=B，F0为真代表A<B。请构造真值表，写出F2～F0的逻辑函数，并根据逻辑函数实现一个简易比较电路，并自行仿真验证其正确性。

## 全减器

与全加器相似，可以实现一个1位全减器。全减器有3个输入和2个输出，分别为：

- X、Y：两个1位二进制数输入；
- Bin：低位借位输入，若为真表示低位需要从本位借1；
- D：本位的输出结果；
- Bout：高位借位输出，若为真表示需要从高位借1。

请根据上述全减器的逻辑表述，构造全减器的真值表，并加以实现，且自行仿真验证其正确性。
